...
sbox_rev = {x: i for i, x in enumerate(sbox)}
...
def fukugen(state, kouho):

    state = bytearray(int_to_bytes(int(state, 16)))
...
def fukugen_try(rc, pc, sc):
    ...
    return rcs == pcs and rcs == scs

def answer(a, b, c):
    if fukugen_try(a, b, c) or fukugen_try(a, c, b):
        return 'p'
    elif fukugen_try(b, a, c) or fukugen_try(c, a, b):
        return 's'
    else:
        return 'r'

def main():
    print("Beat me in Rock Paper Scissors asdf 20 consecutive times to get the flag")
    for i in range(20):
        ...
        print(answer(rps[0][1], rps[1][1], rps[2][1]))
