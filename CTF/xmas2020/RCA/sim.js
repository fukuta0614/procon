...
function xmas(s) {
    var end = 160885440 - start;
    for (var roundNo = s; roundNo <= end; ++roundNo) {
        matrix = nextMatrix(matrix, roundNo);
    }
	var c = document.getElementById('bkgrnd');
	drawScene(c);
...
function proc() {
    ...
    for (var i = 0; i < bits.length; i++){
    	ans += bits[i].toString(16)
	}
    return ans
}


function reverseSquare(x, y) {
	var square = getSquare(x, y);
	var noLiveCells = square.reduce((a, b) => a + b);
	if(noLiveCells != 2) {
		for(var i = 0;i < square.length; ++i) {
...
function prevMatrix(m, roundNo) {
	var startCoord = -(roundNo % 2);

	console.log('Reverse Round #' + roundNo.toString());
	for(var i = startCoord;i < noSquares; i += 2) {
		for(var j = startCoord;j < noSquares; j += 2) {
...
function rev(s){
    for (var roundNo = s; roundNo >= 0; --roundNo) {
        matrix = prevMatrix(matrix, roundNo);
    }
	var c = document.getElementById('bkgrnd');
	drawScene(c);
}
