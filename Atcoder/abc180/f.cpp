...
Factorial factorial(400);

// 組み合わせ
mint nCk(int n, int k, Factorial& f){ return f(n) / (f(n-k) * f(k));}


map<tuple<int, int, int>, mint> memo_f;
map<tuple<int, int, int>, mint> memo_g;

mint f(int n, int m, int l);
mint g(int n, int m, int l);

mint f(int n, int m, int l){

    tuple<int, int, int> key = tie(n, m, l);

    if (memo_f.count(key) > 0){
        return memo_f[key];
    }
    ...
    if (n < l){
        ...
        ret = nCk(n, l, factorial) * g(n-l, m-(l-1), l);
    }

    return memo_f[key] = ret;
