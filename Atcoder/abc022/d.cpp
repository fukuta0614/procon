...
int main() {
    ...
    vector<Point> ch1 = convex_hull(A);
    ch1.emplace_back(ch1[0]);
    double circum1 = 0;
    REP(i, ch1.size()-1) circum1 += (ch1[i+1] - ch1[i]).norm();

    vector<Point> ch2 = convex_hull(B);
    ch2.emplace_back(ch2[0]);
    double circum2 = 0;
    REP(i, ch2.size()-1) circum2 += (ch2[i+1] - ch2[i]).norm();

    print(circum2 / circum1);
