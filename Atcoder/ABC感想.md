...
- B	200th ABC-200
    - 概要
        ...
        - K <= 20
    - 解法
        - 200の倍数じゃなかったときの操作が1000*N+200で、必ず次は200の倍数なので、5*N+1になる。
        - つまり、最大で N * 5^10程度なので、long longで十分とわかる。
    
- C	Ringo's Favorite Numbers 2
    - 概要
        - |A[i] - A[j]|が200の倍数であるようなi, jの組の数
    - 解法
        - i未満で、200で割ったあまりがA[i]%200となる数を足していけば良い。
        - 別解として、200で割ったあまりを数えきったあとに、各rに対して nC2してもよい。
    
- D	Happy Birthday! 2
    - 概要
        - 数列Aに対して、いくつかの要素を選んだときの和を200で割ったあまりを考える。 
        - あまりが一致するような異なる選び方を、実例を含めて答える
    - 解法
        - dp[i][k]: i番目までみたときに、和のあまりがkであるような組み合わせがあるか
        - 上記DPだと、存在確認ができても解の実例が出せない。
        - dp[i][k]: i番目までみたときに、和のあまりがkであるような組み合わせで、最後に選んでindex
        - これで解の復元ができる。初め-1で初期化しておく。
        - k=0の処理でなんか手間取った。
    - 別解
        - 200で割ったあまりが一致する、という条件は実は非常にゆるい。
        - 数列が201個あれば、そのうち1組は必ずあまりがかぶる。鳩の巣原理
        - なので、最初の8個を適当に選んで2^8確認して、一致したやつを答えれば良い。
    
- E	Patisserie ABC 2
    - 概要
        - A,B,Cの指標があり、それぞれ1~Nの値を取ったN^3通りのスコアがあり、
        - A,B,Cの順のキーでソートしたときのK番目のスコアのA,B,Cの値
        - N <= 1e6
    - 解法
        - スコアの和がXとなるようなスコアの数num[X]を考える。 
            - n=3のときにOEISでぐぐったら出てきた。凄まじい関数でこれが解けた
        - X=3から順に足し合わせてKを超えたときのXが、スコアの和
        - 和がわかると、Aの値を決め打ちすればB+C=X-A, B<=NとなるB,Cは計算できるので、これでKまでいけば終わり
    - 別解
        - num[X]はdpで計算できる
        - dp[i][x] : i回選んで、総和がx
- F	Minflip Summation
...
- D	Game in Momotetsu World
    - 概要
        - ゲームDP
        - 二次元マップにプラスます、マイナスますがあり、
          左上から、先手後手が交互に右か下に移動して、マスに応じて得点
        - 右下に到達した時点でゲーム終了で、どっちが勝つか。
    - 解法
        - 状態に先手後手を加えると面倒。
        - dp[i][j] = i,jに到達した人が、それ以降で最適に動いた場合の（自分の得点 - 相手の得点）
        - dp[i][j] = A[i][j] - max(dp[i+1][j], dp[i][j+1])
            - 相手は、相手の得点が最大化する方を選ぶ。それは、自分的には (相手-自分）なので、反転する。 
- E	Xor Distances
    - 概要
        - 木の辺に重みwがあって、 
        - dist[i][j] = i,jをつなぐ最短パスのすべての重みのxor とする
        - sum i,j (dist[i][j]))を求める
    - 解法
        - 根をrとする 
        - dist[i][j] = dist[r][i] ^ dist[r][j]
            - lcaまでの辺は二回通るため、xorは消えるので、一箇所からのdistだけで考えて良い
        - 答えは、sum i,j (dist[r][i] ^ dist[r][j])
            - xorは、bitごとに考えてよくて、そのbitに1が立っている数をa, そうでないのをbとすると、
              n*nのうちa*b個に1が立つ
- F	Insertion Sort
    - 概要
        - 順列Pがあり、昇順に並び替えるコストを最小化
            - コストA[i]で、iを任意の場所に移動
            - コストB[i]で、iを先頭に移動
            - コストC[i]で、iを末尾に移動
    - 解法
        - 全然わからんかった。
    - 考え方
        - 全iについて、A,B,C,動かさないの4通りがある
        - 仮に、動かさないものを決めうったとする。この集合をSとして、数をkとする
            - S1より小さいiは、A,Bのどちらかで移動させる
            - S1 < i < Skに関しては、Aでやるしかない
            - Skより大きいiは、A,Cのどちらか
    - 解法
        - dp[i] = Sの最大をiとしたときに、i以下の数字を並び替えるコストの最小化
            - dp[i] = i未満でSに含まれる最大のidxがjだったとき、の遷移を考える
                - jからi-1までで、動かさないといけないやつは、全部Aで移動させないといけない。
                - i未満でSに含まれるのがない場合は、全部AかBで遷移させる
            - これをいい感じで累積和と、セグ木で高速化
    - 解法2
        - LISっぽく解く
        - https://www.youtube.com/watch?v=sow5naBcXEo&ab_channel=%E3%81%8B%E3%81%A4%E3%81%A3%E3%81%B1%E7%AB%B6%E3%83%97%E3%83%AD
...
- E	Rush Hour 2
    - 概要
        - N点, M辺のグラフで、スタート時間をtとすると、ai -> biの移動は、ci + di / (t+1) かかる
        - 0 -> Nへの最短時間
    - 解法
        - 
- F	Hanjo 2
    - 概要
        - 2x1, 1x1でHxWのマスを埋める方法の数
        - H <= 6, W <= 1e12
    - 解法
        - dp[i][S]: i-1までのマスが埋まっていて、i番目がいくつか埋まっていて、それをbitに表すとSとなるような方法の数
        - dp[i][S] -> dp[i+1][T] の遷移をdfsで求めて、行列累乗
...
- C
    - vectorは a == bで比較できる。知らんかった

- D
    - 相加相乗平均
        - (a + b + c) / 3 >= (a * b * c) ^ 1/3
    - を使うと、x + 1/√x = x + 1/2√x + 1/2√x >= 3/4 になる。面白い。

- E
    - 解法1
        - 数字をひっくり返す操作を一個抜いた時にできる数列は、
            - （例えば1,2,3,4で2,3をひっくり返すのをやめたとき）
        - 抜かなかった場合の数列における2,3をひっくり返したのと同じ。
        - だから、1をひっくり返したかどうかだけで判断可能
    - 解法2
        - 福田解法と発想は同じ。setのswapが高速なこと知らずに無駄に頑張ってしまった。
        - 一応この解法はO(N+MlogN)で福田解法はO(N+M)のはず。
    - 解法3
        - 前から、後ろからでやってく。何度出ても思いつかないなこれ。
        - 結局あみだなので、どこまで行くか、ここからどこまでいくか、は簡単に数列としてあらわせる


### ABC281
- E
    - 概要
        - i = 0 ~ N-M+1に対して、A[i] ~ A[i+M-1]を昇順K番目までの和
    - 解法
        - multisetでがちゃがちゃ。K番目以内のやつとそれ以降のやつをそれぞれで管理
        - multisetのbegin(), rbegin()はO(1)なこと毎回忘れる。
    - 解法2
        - BITを4個つかっても行ける。sumと、数のほうで2つ
    - 解法3
        - 削除予定のやつをpriority_queueで管理してもいい。
        - 削除予定のやつは、即消すんじゃなくて、影響出るタイミングで消せばいい。
- F
    - 概要
        - A[i]のすべての項にあるxとのxorをとったあとの最大値を最小化するx
    - 解法
        - 1000
        - 1011
        - 0111
        - 0011
        - まずは、上から決めたほうがよくて、あるbitに01が混ざってたら、どっちかはそのbitに1が建っちゃう。
        - じゃあそれ以降のbitでの最大値が小さいほうにすればいい。（その時、もうかたっぽは0になるので関係ない）
        - 頑張って再帰にすればいい。
