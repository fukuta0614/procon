// csa_200125_b
// Connect the Graph
// https://csacademy.com/contest/archive/task/connect-the-graph/statement/
#include <bits/stdc++.h>
#ifdef LOCAL
...
int main() {
    ...
    REPN(i, 1, N) {
        ...
        auto e = uf.res.back(); uf.res.pop_back();
        ans.emplace_back(e.first+1, e.second+1, 0+1, i+1);
    ...
    if (possible) {
        print(ans.size());
        for (auto t: ans){
            print(get<0>(t), get<1>(t), get<2>(t), get<3>(t));
        }
    } else {
        print(-1);
    }
