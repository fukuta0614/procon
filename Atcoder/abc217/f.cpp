...
int N2, M;
vector<vector<pair<bool, mint>>> dp;
...
mint rec(int l, int r){
    ...
    //既に計算済み？
    if(dp[l][r].first) return dp[l][r].second;

    //
    if (r - l == 2){
        dp[l][r].first = true;
        int x = AB[l][r-1];
        return dp[l][r].second = x;
    }
    ...
    //パターン２.区間を分ける
    for (int mid = l + 2; mid <= r - 2; mid+=2)
    {
        res += 2 * rec(l, mid) * rec(mid,r);
    }

    dp[l][r].first = true;
    return dp[l][r].second = res;
...
int main() {
    ...
    AB.assign(N2, vector<bool>(N2, false));
    REP(i, M) {
        int a, b;
        cin >> a >> b;
        a--; b--;
    ...
    dp.assign(N2+1, vector<pair<bool, mint>>(N2+1, {false, 0}));
