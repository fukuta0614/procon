...
mint rec(int x, int y){
    ...
    dp[x][y] = 0;
    if (x > A) dp[x][y] += rec(x-1, y) * y;
    if (y > B) dp[x][y] += rec(x, y-1) * x;
    if (x > A && y > B) dp[x][y] -= rec(x-1, y-1)*(x-1)*(y-1);
...
int main() {
    ...
    dp.assign(C+1, vector<mint>(D+1, 0));
    done.assign(C+1, vector<bool>(D+1, false));
