...
bool operator<(const Point& p, double x) {return p.x < x;}
bool operator<(double x, const Point& p) {return x < p.x;}
...
int main() {
    ...
    sort(ALL(points), [](const Point& a, const Point& b){return a.x!=b.x?a.x<b.x:a.y<b.y;});

    auto cmp = [](const Point& a, const Point& b){return a.y!=b.y?a.y<b.y:a.x<b.x;};
    set<Point, decltype(cmp) > cands(cmp);
    cands.insert(Point(INF, -INF));
    cands.insert(Point(INF, INF));
    ...
    REP(i, N){
        while (left < N-1 && points[left].x < points[i].x - R - eps){
            cands.erase(points[left++]);
        }
        auto it = cands.upper_bound(points[i]);

        auto it1 = it;
        auto it2 = (--it);

        if ((*it1-points[i]).norm2() > (*it2-points[i]).norm2()) swap(it1, it2);
        auto p = *it1;

        if ((p - points[i]).norm() > R + eps) {
            ans++;
            cands.insert(points[i]);
        }
    }
