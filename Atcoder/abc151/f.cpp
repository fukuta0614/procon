...
int main() {
    ...
    if (N == 2){
        double ans = (points[0] - points[1]).norm() / 2;
        print(ans);
        return 0;
    }

    double ans = 1e10;
    auto check = [&](Circle tmp){
        bool is_valid = true;
        REP(p, N)  {
            if (not tmp.does_include_point(points[p], true)){
                is_valid = false;
                break;
            }
        }
    ...
    REP(i, N) REPN(j, i+1, N)  {
        ...
        REPN(k, j+1, N){
            Circle tmp;
            bool ret = calc_circle_on_three_points(points[i], points[j], points[k], tmp);
            if (not ret) continue;
            check(tmp);
        }

        // 2点から最小を作成
        double r = (points[i] - points[j]).norm() / 2;
        Point center = points[i] + (points[j] - points[i]) / 2;
        Circle tmp(center, r);
        check(tmp);
    }
