...
// ラグランジュ補間
mint lagrange_interpolation(vector<mint>& Y, int x){
    ...
    REP(i, m){
        ...
        // 分子
        mint numerator = 1;
        REP(k, m) if (k != i) numerator *= mint(x - k);

        // 分母
        mint denominator = 1;
        REP(k, m) if (k != i) denominator *= mint(i - k);

        // 答え
        ret += Y[i] * numerator * denominator.inv();
    }
...
int main() {
    ...
    while (T--){
        ...
        REP(k, p){
            int M = 2*k + 5 + r;
            for(int i=0; 2*i+5<=M; i++) {
                V[k] += nCk(i+4, 4, f) * nCk(M-2*i+5, 10, f);
            }
        }

        mint ans = lagrange_interpolation(V, n);
        print(ans);
    }
