...
//    print(A);
    ...
    REP(i, N){
        ...
        if (i < K){
            ...
            if (min_m != -1 && inc_p == -1 && A[i] > 0){
                inc_p = i;
            }

            if (min_p != -1 && inc_m == -1 && A[i] < 0){
                inc_m = i;
            }
        }
    }
    ...
    if (sign == 0){
        ans = 0;
    } else if (sign > 0){ // 何もしなくて良い
        ;
    } else {
        if (inc_p != -1 && inc_m != -1) {
//            print("a");
            mint a = ans * abs(A[inc_p]) / abs(A[min_m]);
            mint b = ans * abs(A[inc_m]) / abs(A[min_p]);
            if ((ll)abs(A[inc_p]) * abs(A[min_p]) >= (ll)abs(A[inc_m]) * abs(A[min_m])) {
                ans = a;
            } else {
                ans = b;
            }
