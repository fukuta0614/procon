...
int main() {
    ...
    vector<int> place(N+Q+1); // 箱。union findのrootを保持
    REP(i, N+Q+1) place[i] = i;

    vector<int> place_rev(N+Q+1); // rootがどの箱にいるか
    REP(i, N+Q+1) place_rev[i] = i;
    ...
    REP(i, Q){
        ...
        if (t == 1){
            ...
            if (place[y] == -1){
                ...
                uf.unite(place[x], place[y]);
                place[x] = uf.find(place[x]);
                place[y] = -1;
                place_rev[place[x]] = x;
            }
        } else if (t == 2){
