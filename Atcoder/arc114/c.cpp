...
//    }
    ...
    vector<mint> memo2(N+1);
    REP(i, N+1){
        memo2[i] = mint(M).pow(i);
    }
...
//            mint tmp = 0;
//            REP(k, i) tmp += mint(M).pow(i-k-1) * memo[k];
//            ans += (mint(M).pow(i+1) - tmp) * mint(M).pow(N-i-1);
            mint tmp = 0;
            REP(k, i) tmp += memo2[i-k-1] * memo[k];
            ans += (memo2[i+1] - tmp) * memo2[N-i-1];
        }
    }
