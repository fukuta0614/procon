...
int main() {
    ...
    sort(ALL(YAB), greater<tuple<int, int, int>>());

    int Q;
    cin >> Q;
    vector<tuple<int, int, int>> WIV(Q);
    REP(i, Q) {
        int v, w;
        cin >> v >> w;
        v--;
        WIV[i] = make_tuple(w, i, v);
    }
    sort(ALL(WIV), greater<tuple<int, int, int>>());
    ...
    REP(i, Q){
        int w, idx, v;
        tie(w, idx, v) = WIV[i];
        while (tmp < M && get<0>(YAB[tmp]) > w){
            int y, a, b;
            tie(y, a, b) = YAB[tmp];
            uf.unite(a, b);
            tmp++;
        }
        ans[idx] = uf.get_size(v);
    }
